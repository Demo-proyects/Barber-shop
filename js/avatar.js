(function () {
  ...
  const S = {
    ...
    helpAudioActive   : false,  // true mientras ayuda.mp3 está sonando (protege de interrupciones)
  };
  ...
  /* ══════════════════════════════════════════════
     BOOT
     El modal de bienvenida reemplaza los listeners
     de primera interacción — él mismo es la puerta.
  ══════════════════════════════════════════════ */
  function boot() {
  ...
  /* ══════════════════════════════════════════════
     MODAL DE BIENVENIDA
     Primera visita : solo botón OK  → activa avatar + audio intro.
     Visitas siguientes : OK + "Volver a escuchar saludo"
     → replay borra los flags y relanza la secuencia completa.
  ══════════════════════════════════════════════ */
  function showWelcomeModal() {
    const isReturn = !!ls.get('ava_intro_done');
    const replayBtn = welcomeModal.querySelector('#ava-wm-replay');

    // Mostrar el botón de replay solo en visitas posteriores
  ...
  function replayIntro() {
    // Borra ambos flags para relanzar intro + ayuda como si fuera la primera vez
    ...
    firstFired = false;
    killAll();
    // Oculta wrap por si ya estaba visible
  ...
  function setState(mode) {
    ...
    switch (mode) {
      ...
      case 'waiting':
        ...
        // Si ayuda.mp3 está activo, NO lo interrumpimos (solo otro audio lo detiene)
      ...
      case 'help':
        ...
        if (!ls.get('ava_ayuda_done') && !S.audioMain && !S.audioFaq) {
          ...
          playMain(
            C.ayuda,
            /* onPlaying */ () => { img.src = C.speak; S.helpAudioActive = true; },
            /* onEnd     */ () => { S.helpAudioActive = false; if (!S.audioFaq) img.src = C.espera; }
          );
      ...
      case 'faq':
        // Solo interrumpe ayuda.mp3 si el usuario lanza un audio FAQ
        if (!S.helpAudioActive) killMain();
        wrap.classList.add('ava-on');
        hideBtns();
        setPos('center');
  ...
  function killMain() {
    ...
    S.helpAudioActive = false; // siempre limpia el flag al matar explícitamente
  }
  ...
  function buildDOM() {
    ...
    welcomeModal.innerHTML = `
      <div id="ava-wmbox">
        ...
        <div id="ava-wm-body">
          <p id="ava-wm-name"><span class="word-gold">Nara_01</span></p>
          <p id="ava-wm-msg">Tu guía en este sitio.<br>
  ...
  function buildFAQHTML() {
    const items = [
      ...
      { n:2, q:'💰 ¿Cuánto me cuesta probarlo?',
    ...
    const head = `
      <div class="afq-head">
        ...
        <h2 class="font-rye font-extrabold uppercase text-[clamp(1.3rem,2.5vw,1.8rem)] leading-[1.1] text-twht relative z-10">MÁS CLIENTES.<br>
<em class="font-rye gold-text-gradient not-italic text-[clamp(1.5rem,3vw,2.2rem)] leading-[1.3]">DESDE HOY MISMO.</em>
        </h2>
      </div>
      <div class="afq-ctrls">
  ...
  function buildCSS() {
    ...
    /* ── Modal de bienvenida ── */
      ...
      #ava-wmbox{background:#0d0c0a;border:1px solid rgba(191,124,26,.35);padding:28px 28px 24px;max-width:320px;width:86vw;text-align:center;box-shadow:0 28px 72px rgba(0,0,0,.9);position:relative;display:flex;flex-direction:column;align-items:center;gap:18px;}
      #ava-wmbox::before{content:"";position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,#bf7c1a,transparent);}
      #ava-wm-avatar{width:70px;height:70px;border-radius:50%;border:2px solid rgba(191,124,26,.4);overflow:hidden;box-shadow:0 0 0 1px rgba(191,124,26,.15),0 6px 28px rgba(0,0,0,.6);}
      ...
      #ava-wm-name span{color:#bf7c1a;}
      #ava-wm-msg{font-size:.75rem;color:#8a8580;line-height:1.75;margin:0;}
      #ava-wm-msg strong{color:#d6d0c8;}
      #ava-wm-actions{display:flex;flex-direction:column;gap:8px;width:100%;}
      #ava-wm-ok{padding:9px 0;background:transparent;border:1px solid #bf7c1a;color:#bf7c1a;font-size:.72rem;font-weight:700;letter-spacing:.12em;text-transform:uppercase;cursor:pointer;transition:background .18s,color .18s;font-family:inherit;width:100%;}
      #ava-wm-ok:hover{background:rgba(191,124,26,.12);}
      #ava-wm-replay{padding:7px 0;background:transparent;border:1px solid rgba(191,124,26,.25);color:#8a8580;font-size:.68rem;letter-spacing:.06em;cursor:pointer;transition:border-color .2s,color .2s;font-family:inherit;width:100%;align-items:center;justify-content:center;gap:5px;}
      #ava-wm-replay:hover{border-color:#bf7c1a;color:#bf7c1a;}
    `;
  }
